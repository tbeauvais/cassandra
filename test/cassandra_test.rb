...
class CassandraTest < Test::Unit::TestCase
  ...
  def setup
    @twitter = Cassandra1.new('Twitter', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @twitter.clear_keyspace!

    @blogs = Cassandra1.new('Multiblog', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @blogs.clear_keyspace!

    @blogs_long = Cassandra1.new('MultiblogLong', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @blogs_long.clear_keyspace!

    @type_conversions = Cassandra1.new('TypeConversions', "127.0.0.1:9160", :retries => 2, :connect_timeout => 1, :timeout => 5, :exception_classes => [])
    @type_conversions.clear_keyspace!

    Cassandra1::WRITE_DEFAULTS[:consistency] = Cassandra1::Consistency::ONE
    Cassandra1::READ_DEFAULTS[:consistency]  = Cassandra1::Consistency::ONE

    @uuids = (0..6).map {|i| SimpleUUID::UUID.new(Time.at(2**(24+i))) }
    @longs = (0..6).map {|i| Long.new(Time.at(2**(24+i))) }
    @composites = [
      Cassandra1::Composite.new([5].pack('N'), "zebra"),
      Cassandra1::Composite.new([5].pack('N'), "aardvark"),
      Cassandra1::Composite.new([1].pack('N'), "elephant"),
      Cassandra1::Composite.new([10].pack('N'), "kangaroo"),
    ]
    @dynamic_composites = [
      Cassandra1::DynamicComposite.new(['i', [5].pack('N')], ['UTF8Type', "zebra"]),
      Cassandra1::DynamicComposite.new(['i', [5].pack('N')], ['UTF8Type', "aardvark"]),
      Cassandra1::DynamicComposite.new(['IntegerType', [1].pack('N')], ['s', "elephant"]),
      Cassandra1::DynamicComposite.new(['IntegerType', [10].pack('N')], ['s', "kangaroo"]),
    ]
  end
  ...
  def test_setting_default_consistency
    assert_nothing_raised do
      @twitter.default_read_consistency = Cassandra1::Consistency::ALL
    end
    assert_equal(Cassandra1::READ_DEFAULTS[:consistency], Cassandra1::Consistency::ALL)

    assert_nothing_raised do
      @twitter.default_write_consistency = Cassandra1::Consistency::ALL
    end
    assert_equal(Cassandra1::WRITE_DEFAULTS[:consistency], Cassandra1::Consistency::ALL)
  end
...
#  end
  ...
  def test_get_range_reversed
    ...
    hash = Cassandra1::OrderedHash[data]
    reversed_hash = Cassandra1::OrderedHash[data.reverse]
  ...
  if CASSANDRA_VERSION.to_f >= 0.8
    ...
    def test_composite_column_type_conversion
      columns = {}
      @composites.push(
        Cassandra1::Composite.new_from_parts([[20].pack('N'), "meerkat"])
      )
      ...
      columns_in_order = [
        ...
        Cassandra1::Composite.new([5].pack('N'), "zebra"),
        Cassandra1::Composite.new([10].pack('N'), "kangaroo"),
        Cassandra1::Composite.new([20].pack('N'), "meerkat"),
      ]
      ...
      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :start => Cassandra1::Composite.new([1].pack('N')),
        :finish => Cassandra1::Composite.new([20].pack('N'))
      ).keys
      assert_equal(columns_in_order[0..-2], column_slice)

      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :start => Cassandra1::Composite.new([5].pack('N')),
        :finish => Cassandra1::Composite.new([5].pack('N'), :slice => :after)
      ).keys
      assert_equal(columns_in_order[1..2], column_slice)

      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :start => Cassandra1::Composite.new([5].pack('N'), :slice => :after).to_s
      ).keys
      assert_equal(columns_in_order[-2..-1], column_slice)

      column_slice = @type_conversions.get(:CompositeColumnConversion, key,
        :finish => Cassandra1::Composite.new([10].pack('N'), :slice => :before).to_s
      ).keys
    ...
    def test_dynamic_composite_column_type_conversion
      ...
      column_slice = @type_conversions.get(:DynamicComposite, key,
        :start => Cassandra1::DynamicComposite.new(['i', [1].pack('N')]),
        :finish => Cassandra1::DynamicComposite.new(['i', [10].pack('N')])
      ).keys
      assert_equal(columns_in_order[0..-2], column_slice)

      column_slice = @type_conversions.get(:DynamicComposite, key,
        :start => Cassandra1::DynamicComposite.new(['IntegerType', [5].pack('N')]),
        :finish => Cassandra1::DynamicComposite.new(['IntegerType', [5].pack('N')], :slice => :after)
      ).keys
      assert_equal(columns_in_order[1..2], column_slice)

      column_slice = @type_conversions.get(:DynamicComposite, key,
        :start => Cassandra1::DynamicComposite.new(['i', [5].pack('N')], :slice => :after).to_s
      ).keys
      assert_equal([columns_in_order[-1]], column_slice)

      column_slice = @type_conversions.get(:DynamicComposite, key,
        :finish => Cassandra1::DynamicComposite.new(['i', [10].pack('N')], :slice => :before).to_s
      ).keys
      assert_equal(columns_in_order[0..-2], column_slice)
  ...
  def test_keyspace_operations
    system = Cassandra1.new 'system'
    keyspace_name = 'robots'
    keyspace_definition = Cassandra1::Keyspace.new :name => keyspace_name,
    ...
    system.add_keyspace keyspace_definition
    Cassandra1.new(keyspace_name).drop_keyspace
    assert system.keyspaces.none? {|it| it == keyspace_name }
  end
